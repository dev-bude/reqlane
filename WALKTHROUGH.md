# How Reqlane works — a walkthrough
...
- A daemon on `127.0.0.1:7771` owns the state: agents, sessions, requests, messages, artifacts,
  events (SQLite in `~/.reqlane/`). It starts on demand and restarts itself after an upgrade.
- Three agents are registered, each with its repository: `gridlib`, `dashboard` and `po`
  ...
  `/reqlane connect` in a Claude Code session opened in each folder (the hook that handles this
...
```
[reqlane] Agent: dashboard (project), session ses_0011. PO: absent.
[reqlane] Agents in this workspace (you can ask any of them or order work from them):
...
The user does not mention Reqlane, gridlib or any protocol.
...
(`reqlane resolve @gridlib/gridlib/__init__.py`), sees that `set()` renders eagerly, and checks
`reqlane perm check gridlib` → *DENIED (read-only on other repos)*. The card says: never
re-implement another repository's functionality; ask its agent. So it files a request:

```
reqlane req new --to gridlib --type capability --blocking \
...
next: reqlane wait --req req_0002
```
...
1. The `notify:` line is the recorded address of gridlib's session. The dashboard agent sends it
   ...
   `[reqlane] req_0002: capability request from dashboard. Run reqlane inbox` — that wakes the
   gridlib session even if it is idle.
2. The dashboard agent runs `reqlane wait --req req_0002`: a long-poll that returns on the first
...
In the **gridlib** session the cross-session message arrives; the agent runs `reqlane inbox`:
...
`reqlane req show req_0002` prints the request, the references, and `next:` with the full
commands that make sense now (reply, decline, reassign). The agent replies with a clarifying
question (`reqlane reply req_0002 --type clarification --body "..."` — the first reply claims the
request), dashboard's `wait` returns with the message, it answers, and gridlib publishes a
proposal:

```
reqlane propose req_0002 --title "Additive batch-update API, one render per batch" \
...
How to solve the problem — the API shape, the implementation — is gridlib's decision. Reqlane only
...
```
reqlane req accept req_0002 --option B
```
```
...
```
reqlane deliver req_0002 --repo gridlib --commit 5ae7294 --version 1.1.0 \
...
```
reqlane evaluate req_0002 --verdict accepted --data '{"before_ms":10388,"after_ms":6}' --body -
```
```
...
`reqlane req show req_0002` — or the browser page from `/reqlane ui` — now shows the whole
...
- The user said "hand it to the PO" → `reqlane handoff req_0003`. Opening the `product` folder in
  Claude Code connected the PO; `reqlane po dashboard` listed the handed-over decision first, the PO
  agent read `PRODUCT.md`, proposed option A to the user (hybrid policy: breaking changes are put to
  the human first) and recorded `reqlane decide req_0003 --option A --reason "1.x API frozen" --affected gridlib,dashboard`.
...
The PO can also start work itself: `reqlane po task dashboard --title "..."` puts a `task` in
...
Seen: one instruction; optionally one decision. The browser page (`/reqlane ui`) shows each request
...
- The card handed to agents: `reqlane protocol` (owned by the daemon; a runtime carries only a
